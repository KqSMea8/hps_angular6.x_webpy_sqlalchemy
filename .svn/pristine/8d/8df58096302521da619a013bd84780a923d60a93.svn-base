...
export class HttpService {
    ...
    request<T>(sMethon: string, sPath: string, objParams?: object, isFormData: boolean = false): Observable<T> {
        if (sMethon.length === 0) {
            ...
            if (sMethon.toLowerCase() === 'get') {
                ...
                if (isFormData) {
                    return this.m_objHttpClient.post<T>(environment.baseUrl + sPath, objParams);
                } else {
