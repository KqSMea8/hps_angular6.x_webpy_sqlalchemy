...
export class HotelAddComponent implements OnInit {
    ...
    fnAdd(): void {
        ...
        this.m_objApiService.fnAddHotel(this.objAddHotelCondition).subscribe(data => {
            if (data.Code === 200) {
                this.m_objApiService.fnSaveImgHotelID(this.objSaveImgPath).subscribe(datas => {
                        if (datas.Code === 200) {
                            this.m_objToolsService.fnDoSuccessReturnTips('添加成功');
                            this.m_objRouter.navigate(['/home/hotelManagement/verify']);
                        }
                    }
                );
            } else {
                this.m_objToolsService.fnErrorReturnTips('有必填项未完成');
            }
        });
    }
        ...
        this.m_objApiService.fnVerifyHotel(this.objVerifyHotelCondition).subscribe(data => {
            if (data.Code === 200) {
                ...
                window.setTimeout(function () {
                    that.m_objRouter.navigate(['/main/hotelManagement/verify'])
                }, 2000);
            } else {
    ...
    fnUnVerifyHotel(): void {
        ...
        this.objVerifyHotelCondition.nState = 1;
        this.objVerifyHotelCondition.sRemark = this.objAddHotelCondition.sRemark;
        this.objVerifyHotelCondition.nAuditState = 3;
        this.isVisible = false;
        this.m_objApiService.fnVerifyHotel(this.objVerifyHotelCondition).subscribe(data => {
            if (data.Code === 200) {
                this.fnGetHotelDetail(this.nHotelID);
                ...
                let that = this;
                window.setTimeout(function () {
                    that.m_objRouter.navigate(['/home/hotelManagement/verify'])
                }, 2000);
            } else {
                this.m_objToolsService.fnErrorReturnTips(data.Msg);
            }
    ...
    SpecialIndustriesDelPic = (file: UploadFile) => {
        ...
        const index = file.url.indexOf('Path=');
        if (index !== -1) {
    ...
    SpecialIndustriesCustomReq = (item: UploadXHRArgs) => {
        ...
        const url = environment.baseUrl + '/Hotel/UploadImg';
        this.m_objHttpClient.post(url, formData).subscribe(data => {
            const len = this.SpecialIndustriesFileList.length - 1;
            const objLast = this.SpecialIndustriesFileList[len];
            ...
            this.m_objApiService.fnSaveImgPath(this.objSaveImgPath).subscribe(datas => {
                if (datas.Code === 200) {
    ...
    HygienePermitCustomReq = (item: UploadXHRArgs) => {
        ...
        const url = environment.baseUrl + '/Hotel/UploadImg';
        this.m_objHttpClient.post(url, formData).subscribe(data => {
            const len = this.HygienePermitFileList.length - 1;
            const objLast = this.HygienePermitFileList[len];
            this.objSaveImgPath.CertificatesURL = data['basePath'];
            this.objSaveImgPath.UID = objLast['uid'];
    ...
    FirePermitCustomReq = (item: UploadXHRArgs) => {
        ...
        const url = environment.baseUrl + '/Hotel/UploadImg';
        this.m_objHttpClient.post(url, formData).subscribe(data => {
            const len = this.FirePermitFileList.length - 1;
            const objLast = this.FirePermitFileList[len];
            this.objSaveImgPath.CertificatesURL = data['basePath'];
            this.objSaveImgPath.UID = objLast['uid'];
    ...
    BusinessPreviewCustomReq = (item: UploadXHRArgs) => {
        ...
        const url = environment.baseUrl + '/Hotel/UploadImg';
        this.m_objHttpClient.post(url, formData).subscribe(data => {
            const len = this.BusinessPreviewFileList.length - 1;
            const objLast = this.BusinessPreviewFileList[len];
            this.objSaveImgPath.CertificatesURL = data['basePath'];
            this.objSaveImgPath.UID = objLast['uid'];
            ...
            // this.objSaveImgPath.IsFront = 4;
            // this.objSaveImgPath.HotelID = 5;
            this.m_objApiService.fnSaveImgPath(this.objSaveImgPath).subscribe(DataSource => {
    ...
    /**取消按钮 -> 返回酒店搜索页面 */
    goHotelSearch(): void {
        this.m_objRouter.navigate(['/main/Hotel/HotelSearch'])
    }
